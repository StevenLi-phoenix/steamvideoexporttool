...
class SteamMetadataTests(unittest.TestCase):
    ...
    def test_local_manifest_and_online_fallback_are_used_when_metadata_is_missing(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            manifest.unlink()
            with (
                patch("steam_exporter.media._steam_roots", return_value=[]),
                patch("steam_exporter.media._fetch_steam_game_name", return_value="Online Portal"),
            ):
...
class ProcessHelperTests(unittest.TestCase):
    ...
    def test_extract_preview_frames_creates_four_thumbnails_at_even_offsets(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch("steam_exporter.media.find_executable", side_effect=lambda name, ffmpeg_path=None: Path(f"{name}.exe")),
